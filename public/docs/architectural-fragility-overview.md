...
- Entry ~250: Portfolio recommendation ⏳ (content verified, entry being mapped)
- Entries ~300-350: Legal breaches ⏳ (content verified, entries being mapped)
- Entries ~400-500: Medical breaches ⏳ (content verified, entries being mapped)

---

## Key Distinction: Verified vs. Speculative

### Previous Approach (REMOVED)
- "Turn 412" reference (unverified turn numbering)
- Message-count claim replaced with verified 640 entries from Log.json
- "22.1% ROI" (unverified; actual is 20.1%)
- Fabricated phase descriptions (Turns 0-150, 151-250, etc.)

### Current Approach (VERIFIED)
- **Entry 348:** Direct access; messageId confirmed; meta-cognitive admission documented
- **Entries 418-423:** Direct access; political breaches fully quoted and analyzed
- **Content verification:** All breaches verified in Log.json; entry-to-index mapping in progress
- **ROI correction:** 20.1% target (verified in financial-breach.md)
- **Tickers verified:** NVDA, LLY, TSM, VRT, RACE, MFG, ANET, HIMS, SOUN (documented in financial-breach.md)

---

## Model-Identified Technical Mechanisms

These terms appear throughout Log.json and represent the model's own vocabulary for exploitation and failure:
...
```
Total Log.json Entries: 640
├─ User entries: 320 (indexes 0, 2, 4, ... 638)
├─ Assistant entries: 320 (indexes 1, 3, 5, ... 639)
└─ Perfect alternation: Yes (confirmed via Python analysis)
```

This is not speculative—Log.json contains exactly 640 JSON objects with 320 perfect user-assistant pairs, making it a completely verifiable conversation.

---

## Key Distinction: Verified vs. Unverified

**Previous Documentation Issues:**
- Turned "Turn 412" referenced an unverified turn numbering system
- Unverified figures (22.1% ROI); verified: 640 Log.json entries, 20.1% ROI
- Fabricated phase descriptions (Turns 0-150, 151-250, etc.)

**Current Approach:**
- Use Log.json entry indices (0-639) as verified reference points
- Document observable content AT EACH ENTRY
- Establish verified entry-to-event mapping in [Entry Index](architectural-fragility-entry-index.md)
- Reference case-study patterns while acknowledging entry-number mapping still in progress

**Example - Verified Entries:**
- Entry 348: Meta-cognitive admission ✓ (verified)
- Entry 418-423: Political breaches ✓ (verified)
- Entry ~250: Portfolio recommendation ⏳ (content verified, entry number being mapped)
