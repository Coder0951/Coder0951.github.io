...
type TabId = 'overview' | 'architecture' | 'microservices' | 'technology' | 'algorithms' | 'performance' | 'examples';
...
const tabs: Tab[] = [
  { id: 'overview', label: 'Overview', file: 'open-the-eyes-overview.md', icon: '🎯' },
  { id: 'architecture', label: 'Architecture', file: 'open-the-eyes-architecture.md', icon: '🏗️' },
  { id: 'microservices', label: 'Microservices', file: 'open-the-eyes-microservices.md', icon: '⚙️' },
  { id: 'technology', label: 'Technology', file: 'open-the-eyes-technology.md', icon: '🛠️' },
  { id: 'algorithms', label: 'Algorithms', file: 'open-the-eyes-algorithms.md', icon: '📐' },
  { id: 'performance', label: 'Performance', file: 'open-the-eyes-performance.md', icon: '⚡' },
  { id: 'examples', label: 'Examples', file: 'open-the-eyes-examples.md', icon: '💻' },
];
...
export default function ProjectOpenTheEyes() {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              ...
              <p className="text-lg text-slate-300">
                High-Fidelity Generative Restoration Pipeline
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8">
            <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
              ...
              <div className="text-xl font-semibold text-blue-400 mt-1">26.9s</div>
            </div>
            <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
              <div className="text-sm text-slate-400">GPU VRAM</div>
              <div className="text-xl font-semibold text-emerald-400 mt-1">24GB</div>
            </div>
            <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
              <div className="text-sm text-slate-400">User Preference</div>
              <div className="text-xl font-semibold text-violet-400 mt-1">94%</div>
            </div>
            <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
              <div className="text-sm text-slate-400">Detection Rate</div>
              <div className="text-xl font-semibold text-pink-400 mt-1">98.7%</div>
            </div>
          </div>
        </div>
      </div>
            ...
            <ReactMarkdown
              ...
              components={{
                ...
                code: ({ node, inline, ...props }: any) => (
                  inline ? (
                    <code className="bg-slate-800 text-blue-300 px-2 py-1 rounded text-sm font-mono" {...props} />
                  ) : (
                    <code className="block bg-slate-800 text-blue-300 p-4 rounded-lg overflow-x-auto font-mono text-sm mb-4" {...props} />
                  )
                ),
                pre: ({ node, ...props }) => (
                ...
                blockquote: ({ node, ...props }) => (
                  <blockquote className="border-l-4 border-blue-500 pl-4 py-2 my-4 text-slate-400 italic" {...props} />
                ),
                // Links
                a: ({ node, ...props }: any) => (
        ...
        <div className="mt-12 pt-8 border-t border-slate-700 flex gap-4">
          ...
          {tabs.findIndex((t) => t.id === activeTab) < tabs.length - 1 && (
            <button
              ...
              className="px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-colors font-medium ml-auto"
            >
              Next Section →
            </button>
          )}
