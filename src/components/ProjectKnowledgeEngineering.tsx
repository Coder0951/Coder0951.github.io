...
type TabId = 'overview' | 'architecture' | 'personas' | 'knowledge-base' | 'algorithms' | 'performance' | 'examples';
...
const tabs: Tab[] = [
  { id: 'overview', label: 'Overview', file: 'knowledge-engineering-overview.md', icon: '🎯' },
  { id: 'architecture', label: 'Architecture', file: 'knowledge-engineering-architecture.md', icon: '🏗️' },
  { id: 'personas', label: 'Personas', file: 'knowledge-engineering-personas.md', icon: '👥' },
  { id: 'knowledge-base', label: 'Knowledge Base', file: 'knowledge-engineering-knowledge-base.md', icon: '📚' },
  { id: 'algorithms', label: 'Algorithms', file: 'knowledge-engineering-algorithms.md', icon: '⚙️' },
  { id: 'performance', label: 'Performance', file: 'knowledge-engineering-performance.md', icon: '⚡' },
  { id: 'examples', label: 'Examples', file: 'knowledge-engineering-examples.md', icon: '💻' },
];
...
export default function ProjectKnowledgeEngineering() {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              ...
              <p className="text-lg text-slate-300">
                Persona-Based Career Logic & Dossier Synthesis
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8">
            <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
              ...
              <div className="text-xl font-semibold text-emerald-400 mt-1">0%</div>
            </div>
            <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
              <div className="text-sm text-slate-400">Factual Accuracy</div>
            ...
            <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
              ...
              <div className="text-xl font-semibold text-blue-400 mt-1">&lt;2 sec</div>
            </div>
            <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
              <div className="text-sm text-slate-400">Recruiter Quality</div>
              <div className="text-xl font-semibold text-violet-400 mt-1">9.2/10</div>
            </div>
          </div>
        </div>
      </div>
            ...
            <ReactMarkdown
              ...
              components={{
                ...
                blockquote: ({ node, ...props }) => (
                  <blockquote className="border-l-4 border-violet-500 pl-4 py-2 my-4 text-slate-400 italic" {...props} />
                ),
                // Links
                a: ({ node, ...props }: any) => (
                  <a className="text-violet-400 hover:text-violet-300 underline transition-colors" {...props} />
                ),
        ...
        <div className="mt-12 pt-8 border-t border-slate-700 flex gap-4">
          ...
          {tabs.findIndex((t) => t.id === activeTab) < tabs.length - 1 && (
            <button
              ...
              className="px-6 py-2 bg-violet-600 hover:bg-violet-500 text-white rounded-lg transition-colors font-medium ml-auto"
            >
              Next Section →
            </button>
          )}
